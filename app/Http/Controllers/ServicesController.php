...
class ServicesController extends Controller
{
    ...
    public function showStat (Request $request){
        ...
        $servicesQuery = Services::where('user_id', $user->id);

        // Apply filter by 'name' if provided
        if ($request->has('name') && !empty($request->name)) {
            $servicesQuery->where('name', 'like', '%' . $request->name . '%');
        }

        // Apply filter by 'quantity' if provided
        if ($request->has('quantity') && !empty($request->quantity)) {
            $servicesQuery->where('quantity', '<=' ,  $request->quantity);
        }

        // Apply filter by 'price' if provided
        if ($request->has('price') && !empty($request->price)) {
            $servicesQuery->where('price' , '<=' , $request->price);
        }
        ...
        $commandeTotalPerStock = function($serviceId , $what_to_return) 
        {
        ...
        return view('users.statistiques.services', compact('services' , 'commandeTotalPerStock' , 'clientList', 'hasPhysique', 
